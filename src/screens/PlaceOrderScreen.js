...
export const PlaceOrderScreen = ({ history }) => {
  ...
  cart.shippingPrice = addDecimals(cart.temsPrice > 100 ? 0 : 100);
  cart.taxPrice = addDecimals(Number((0.15 * cart.itemsPrice).toFixed(2)));
  cart.totalPrice = (
    Number(cart.itemsPrice) + Number(cart.shippingPrice)
  ).toFixed(2);

  cart.payprice = (Number(cart.itemsPrice) + Number(cart.shippingPrice)) * 100;
          ...
          <ListGroup variant="flush">
            <ListGroup.Item>
              ...
              <p style={{ lineHeight: "2.0" }}>
                <strong style={{ fontWeight: "500" }}>Address : </strong> <br />
